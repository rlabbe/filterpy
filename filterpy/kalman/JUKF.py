    ...
    **References**
    ...
    def update(self, z, R=None, UT=None, hx_args=(),
        ...
        **Parameters**
        ...
        '''for i in range(self._num_sigmas):
            self.Pxz += self.W[i] * np.outer(residual_x(sigmas_f[i], self.x),
                                             residual_h(sigmas_h[i], zp))'''

        # this is the unreadable but fast implementation of the
        # commented out loop above
        yh = residual_x(sigmas_f, self.x[np.newaxis, :])
        yz = residual_h(sigmas_h, zp[np.newaxis, :])
    ...
    def predict(self, dt=None,  UT=None, fx_args=()):
        ...
        self.x, self.P = UT(self.sigmas_f, self.W, self.W, self.Q)
